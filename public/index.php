        ...
        if (isset($_POST['botonCreaNotificacion'])) {
            if ($_POST['contrato'] == 0) {
                $contratos = ORM::for_table('contrato')
                    ->find_many();
                $app->render('nueva_notificacion.html.twig', [
                    'mensajeError' => 'Es necesario seleccionar un contrato',
                    'contratos' => $contratos
                ]);
                die();
            }
            if ($_POST['socio'] == 0) {
                $contratos = ORM::for_table('contrato')
                    ->find_many();
                $app->render('nueva_notificacion.html.twig',
                    ['mensajeError' => 'Es necesario seleccionar un socio para el contrato',
                        'contratos' => $contratos
                ]);
                die();
            } else {
                if ($_POST['cisternas'] == 0) {
                    $contratos = ORM::for_table('contrato')
                        ->find_many();
                    $app->render('nueva_notificacion.html.twig', [
                        'mensajeError' => 'Debe de seleccionar las cisternas obligatoriamente',
                        'contratos' => $contratos
                    ]);
                    die();
                }
                ...
                if ($_POST['cisternas'] > 1) {
                    ...
                    if ($compMat == $_POST['cisternas'] || $compMat == 0) {
                        ...
                        $contratos = ORM::for_table('contrato')
                            ->find_many();
                        $app->render('nueva_notificacion.html.twig', [
                            'mensajeError' => 'No puede dejar campos vacíos en las matrículas',
                            'contratos' => $contratos
                        ]);
                        die();
                    }
                ...
                $app->render('inicio.html.twig', [
                    'mensajeOk' => 'Notificación agregada con éxito'
                ]);
                die();
            }
        };
